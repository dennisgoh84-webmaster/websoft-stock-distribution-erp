                ...
                <form method="POST" action="{{ route('stock-transfers.store') }}" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select id="from_warehouse_id" name="from_warehouse_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                        ...
                        <div>
                            ...
                            <select id="to_warehouse_id" name="to_warehouse_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                <option value="">{{ __('Select warehouse') }}</option>
                                @foreach ($warehouses as $warehouse)
                        ...
                        <table class="min-w-full divide-y divide-gray-200 border">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 text-right text-xs font-medium text-gray-500 uppercase w-32">{{ __('Quantity') }}</th>
                                    <th class="w-10"></th>
                                </tr>
                            </thead>
                            <tbody>
                    ...
                    <p class="text-xs text-gray-500">{{ __('Transfers apply immediately: stock moves out of the source warehouse and into the destination warehouse.') }}</p>
