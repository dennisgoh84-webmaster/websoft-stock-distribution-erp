<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-slate-900 leading-tight tracking-tight">{{ __('Warehouses') }}</h2>
            <a href="{{ route('warehouses.create') }}" class="inline-flex items-center gap-x-1.5 px-4 py-2 bg-amber-600 border border-transparent rounded-lg font-semibold text-sm text-white shadow-sm hover:bg-amber-700 transition ease-in-out duration-150">
                {{ __('New Warehouse') }}
            </a>
        </div>
    </x-slot>
                    ...
                    <tbody class="bg-white divide-y divide-slate-200">
                        @forelse ($warehouses as $warehouse)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-sm text-slate-500">{{ $warehouse->code }}</td>
                                <td class="px-6 py-4 text-sm text-slate-500">{{ $warehouse->products_count }}</td>
                                ...
                                <td class="px-6 py-4 text-right text-sm space-x-3">
                                    <a href="{{ route('warehouses.edit', $warehouse) }}" class="text-amber-700 hover:text-amber-900">{{ __('Edit') }}</a>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-sm text-slate-500 text-center">{{ __('No warehouses yet.') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
