<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-slate-900 leading-tight tracking-tight">{{ __('New Stock Adjustment') }}</h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm ring-1 ring-slate-900/5 sm:rounded-xl p-6">
                <form method="POST" action="{{ route('stock-adjustments.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <table class="min-w-full divide-y divide-slate-200 border">
                            <thead class="bg-slate-50">
                                <tr>
                                    ...
                                    <th class="px-3 py-2 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider w-40">{{ __('Quantity Change') }}</th>
                                    <th class="w-10"></th>
                                </tr>
                            </thead>
                            <tbody>
                        ...
                        <p class="mt-2 text-xs text-slate-500">{{ __('Use a positive number to increase stock, negative to decrease it.') }}</p>
                    </div>
