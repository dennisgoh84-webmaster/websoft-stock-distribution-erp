    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm ring-1 ring-slate-900/5 sm:rounded-xl p-6 grid grid-cols-1 sm:grid-cols-4 gap-4 text-sm">
                <div>
                    <span class="text-slate-500 block">{{ __('Supplier') }}</span>
                    <a href="{{ route('suppliers.show', $voucher->supplier) }}" class="hover:underline">{{ $voucher->supplier->name }}</a>
                </div>
                <div><span class="text-slate-500 block">{{ __('Payment Date') }}</span> {{ $voucher->payment_date->format('Y-m-d') }}</div>
                <div><span class="text-slate-500 block">{{ __('Method') }}</span> {{ str($voucher->method)->replace('_', ' ')->title() }}</div>
                <div><span class="text-slate-500 block">{{ __('Reference No.') }}</span> {{ $voucher->reference_no ?? '—' }}</div>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm ring-1 ring-slate-900/5 sm:rounded-xl p-6 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                <div><span class="text-slate-500 block">{{ __('Amount Paid') }}</span> <span class="text-lg font-semibold">{{ number_format($voucher->amount, 2) }}</span></div>
                <div><span class="text-slate-500 block">{{ __('Allocated') }}</span> <span class="text-lg">{{ number_format($voucher->allocatedAmount(), 2) }}</span></div>
                <div><span class="text-slate-500 block">{{ __('Unallocated') }}</span> <span class="text-lg font-semibold">{{ number_format($voucher->unallocatedAmount(), 2) }}</span></div>
            </div>
            ...
            @if ($voucher->unallocatedAmount() > 0.001)
                <div class="bg-white overflow-hidden shadow-sm ring-1 ring-slate-900/5 sm:rounded-xl p-6">
                    <h3 class="font-semibold text-slate-700 mb-4">{{ __('Allocate to a Bill') }}</h3>

                    @if ($openInvoices->isEmpty())
                        <p class="text-sm text-slate-500">{{ __('This supplier has no outstanding purchase invoices to allocate against.') }}</p>
                    @else
