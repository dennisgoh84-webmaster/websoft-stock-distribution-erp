    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 grid grid-cols-1 sm:grid-cols-4 gap-4 text-sm">
                <div>
                    <span class="text-gray-500 block">{{ __('Customer') }}</span>
                    <a href="{{ route('customers.show', $receipt->customer) }}" class="hover:underline">{{ $receipt->customer->name }}</a>
                </div>
                <div><span class="text-gray-500 block">{{ __('Receipt Date') }}</span> {{ $receipt->receipt_date->format('Y-m-d') }}</div>
                <div><span class="text-gray-500 block">{{ __('Method') }}</span> {{ str($receipt->method)->replace('_', ' ')->title() }}</div>
                <div><span class="text-gray-500 block">{{ __('Reference No.') }}</span> {{ $receipt->reference_no ?? '—' }}</div>
            </div>

            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                <div><span class="text-gray-500 block">{{ __('Amount Received') }}</span> <span class="text-lg font-semibold">{{ number_format($receipt->amount, 2) }}</span></div>
                <div><span class="text-gray-500 block">{{ __('Allocated') }}</span> <span class="text-lg">{{ number_format($receipt->allocatedAmount(), 2) }}</span></div>
                <div><span class="text-gray-500 block">{{ __('Unallocated (on account)') }}</span> <span class="text-lg font-semibold">{{ number_format($receipt->unallocatedAmount(), 2) }}</span></div>
            </div>
            ...
            @if ($receipt->unallocatedAmount() > 0.001)
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <h3 class="font-semibold text-gray-700 mb-4">{{ __('Allocate to an Invoice') }}</h3>

                    @if ($openInvoices->isEmpty())
                        <p class="text-sm text-gray-500">{{ __('This customer has no outstanding sales invoices to allocate against.') }}</p>
                    @else
